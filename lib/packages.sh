...
APT_LIST_BACKUP_DIR_FOR_VMANAGER="${CACHE_DIR}/var-lib-apt-for-vmanager"
APT_LIST_BACKUP_DIR_FOR_OTHER="${CACHE_DIR}/var-lib-apt-for-other"
LAST_APT_UPDATE_DATE_FILE_FOR_VMANAGER="${CACHE_DIR}/last-apt-update-for-vmanager"
...
VMANAGER_PACKAGE_NAME="virt-manager"

# SourceList is to replace /etc/apt/sources.list
# SourceParts=- is to disable /etc/apt/sources.list.d
APT_OPTS_FOR_VMANAGER="-o Dir::Etc::SourceList=${CLEARNET_VMANAGER_LIB_HOME}/hiddenvm.list -o Dir::Etc::SourceParts=-"

#update_package_lists_for_vmanager_or_restore_cache
update_package_list_for_vmanager_or_restore_cache() {
    # Update + back up package indexes for linux-headers and vmanager, or use the cache
    if should_apt_update_for_vmanager; then
        log "Run 'apt-get update' for linux-headers and vmanager, prog-id=7"
        run_apt_update_for_vmanager
        back_up_apt_lists_for_vmanager
    else
        log "Restore package index cache for linux-headers and vmanager, prog-id=8"
        restore_apt_list_cache_for_vmanager
...
install_packages() {
    ...
    #   ebtables was added for testing purposes to fix a firewall issue with libvirt then removed
    log "Install packages, prog-id=9"
    sudo apt-get -q -y ${APT_OPTS_FOR_VMANAGER} install make bindfs dpkg-dev qemu-kvm libvirt-daemon-system libvirt-clients gir1.2-spiceclientgtk-3.0 dnsmasq qemu-utils bridge-utils

    # Install linux-headers
    log "Installing ${LINUX_HEADERS_PKG} from '${LINUX_HEADERS_RELEASE}', prog-id=10"
    sudo DEBIAN_FRONTEND=noninteractive apt-get -q -y --no-install-recommends ${APT_OPTS_FOR_VMANAGER} \
        -t ${LINUX_HEADERS_RELEASE} install ${LINUX_HEADERS_PKG}

    # Install Virtual Machine Manager (note that this will create the vmanagerusers group and
    # build/install the Virtual Machine Manager kernel drivers)
    log "Install Virtual Machine Manager, prog-id=11"
    sudo DEBIAN_FRONTEND=noninteractive apt-get -q -y ${APT_OPTS_FOR_VMANAGER} install ${VMANAGER_PACKAGE_NAME}
}
...
downgrade_packages_for_stability() {
    ...
    then
        ...
        sudo DEBIAN_FRONTEND=noninteractive apt-get -q -y --allow-downgrades ${APT_OPTS_FOR_VMANAGER} install ${DOWNGRADE_STR}
    else
...
update_package_list_for_other_uses_or_restore_cache() {
    # Update + back up package indexes for things other than linux-headers and vmanager, or use the cache
...
should_apt_update_for_vmanager() {
    should_apt_update "${APT_LIST_BACKUP_DIR_FOR_VMANAGER}" "${LAST_APT_UPDATE_DATE_FILE_FOR_VMANAGER}"
}
...
should_prune_package_cache() {
    ...
    if [ "${NUM_DAYS}" -ge "${PKG_CACHE_PRUNE_FREQ_DAYS}" ]; then
        log "It's been at least ${PKG_CACHE_PRUNE_FREQ_DAYS} days since the package cache was last pruned on (${LAST_PKG_CACHE_PRUNE_DATE_FILE})"
        return 0
    fi
...
run_apt_update_for_vmanager() {
    run_apt_update "${APT_OPTS_FOR_VMANAGER}" "${LAST_APT_UPDATE_DATE_FILE_FOR_VMANAGER}"
}
...
restore_apt_list_cache_for_vmanager() {
    restore_apt_list_cache "${APT_LIST_BACKUP_DIR_FOR_VMANAGER}"
}
...
back_up_apt_lists_for_vmanager() {
    back_up_apt_lists "${APT_LIST_BACKUP_DIR_FOR_VMANAGER}"
}
...
back_up_apt_packages() {
    log "Back up apt packages, prog-id=22"
