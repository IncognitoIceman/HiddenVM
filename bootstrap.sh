...
+------------------------------------------------------------------------------------+
| This software is not affiliated with the Tails or Virtual Machine Manager projects |
+------------------------------------------------------------------------------------+
...
# Make sure the CLEARNET_VMANAGER_LIB_HOME directory exists and is clean

rm -rf "${CLEARNET_VMANAGER_LIB_HOME}"
mkdir -p "${CLEARNET_VMANAGER_LIB_HOME}"

log "Copy files to ${CLEARNET_VMANAGER_LIB_HOME}, prog-id=2"

# Install some scripts/libraries to CLEARNET_VMANAGER_LIB_HOME to support some
# HiddenVM features that can launch from outside the AppImage mount, such as
# the user manually launching Clearnet Virtual Machine Manager.
cp "lib/common.sh" "${CLEARNET_VMANAGER_LIB_HOME}"
cp "lib/clearnet-vmanager.sh" "${CLEARNET_VMANAGER_LIB_HOME}"
chmod +x "${CLEARNET_VMANAGER_LIB_HOME}/clearnet-vmanager.sh"
...
cp "lib/assets/hiddenvm.list" "${CLEARNET_VMANAGER_LIB_HOME}"
cp "lib/never-ask-password.sh" "${CLEARNET_VMANAGER_LIB_HOME}"
chmod +x "${CLEARNET_VMANAGER_LIB_HOME}/never-ask-password.sh"
...
echo "${ADMIN_PASS}" | sudo -S "${CLEARNET_VMANAGER_LIB_HOME}/never-ask-password.sh"
...
cat > "${CLEARNET_VMANAGER_ENV_FILE}" <<EOF
HVM_HOME="${HVM_HOME}"
...
cp "${CLEARNET_VMANAGER_ENV_FILE}" "${HVM_HOME}/env"
...
# Delete the default virt-manager launcher to prevent user confusion
sudo rm -f /usr/share/applications/virt-manager.desktop

# Launch Clearnet Virtual Machine Manager in the background using sudo, preventing
# the parent shell from killing it (nohup) if terminated
log "Launching Clearnet Virtual Machine Manager, prog-id=18"
nohup bash -c "${CLEARNET_VMANAGER_LIB_HOME}/clearnet-vmanager.sh < /dev/null &" \
    >/dev/null 2>&1
...
if should_prune_package_cache; then
    log "Recomputing list of unused cached packages, prog-id=21"
...
log "Done! Runtime: ${SECONDS}s, prog-id=23"
